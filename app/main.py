...
INTERCOM_URL = "https://api.intercom.io/users/?user_id={}"
...
async def intercom_request(url: str, data: Optional[dict] = None, method: str = "GET"):
    ...
    if not (method == "POST" and not IC_TOKEN):
        r = session.request(
            method, "https://api.intercom.io" + url, json=data, headers=headers
        )
        r.raise_for_status()
        return r.json()
...
async def check_support_reply(item: dict):
    ...
    if support_level == "No Support":
        ...
        await intercom_request(
            f"/conversations/{item['id']}/reply/", data=reply_data, method="POST"
        )
...
app = Starlette(
    ...
    routes=[Route("/", index), Route("/callback/", callback)],
)
